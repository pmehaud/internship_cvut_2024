...
def plotMap():
    ...
    if(selected_option in action_options):
        ...
        map = mu.getMap(data = copy.deepcopy(df), selected_providers = selected_providers, selected_regions = selected_regions, selected_technologies = [f'site_{st.lower()}' for st in selected_technologies], option = action_var.get(), epsilon=float(epsilon_entry.get()), nmin=int(n_min_entry.get()))
...
technology_checkboxes = [ctk.CTkCheckBox(selection_frame, text=tech, font=("Helvetica", 20), variable=technology_vars[i], checkbox_height=30, checkbox_width=30) for i, tech in enumerate(technologies)]
